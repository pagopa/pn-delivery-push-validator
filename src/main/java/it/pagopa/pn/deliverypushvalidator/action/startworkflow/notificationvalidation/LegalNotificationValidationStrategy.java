...
/**
 * Validation strategy for LEGAL notifications.
 * Encapsulates the original validation logic from NotificationValidationActionHandler.
 */
@Component
@AllArgsConstructor
@CustomLog
public class LegalNotificationValidationStrategy implements NotificationValidationStrategy {
    ...
    public void validate(NotificationInt notification, NotificationValidationActionDetails details) {
        log.debug("Start LEGAL validateNotification - iun={}", notification.getIun());
    ...
    // ---- private helpers (moved from handler) ----
    ...
    private boolean f24Exists(NotificationInt notification) {
        return notification.getRecipients()
                ...
                .anyMatch(payments -> !CollectionUtils.isEmpty(payments)
                        && payments.stream().anyMatch(p -> p.getF24() != null));
    }
    ...
    private void verifyLookUpAddressAndNormalizeAddress(NotificationInt notificationInt) {
        ...
        if (notificationInt.getUsedServices() != null && notificationInt.getUsedServices().getPhysicalAddressLookUp()) {
            ...
            try {
                lookupAddressHandler.performValidation(notificationInt, true);
    ...
    private void quickWorkAroundForPN9116(NotificationInt notification) {
        if (!parameterConsumer.isPaEnabledToSendMoreThan20Grams(notification.getSender().getPaTaxId())) {
            ...
            switch (numberOfDocuments) {
                ...
                default -> throw new PnValidationMoreThan20GramsException(errorDetail + " " + numberOfDocuments + " documents and an AAR exceed 3 sheets");
            }
        }
    }
        ...
        for (NotificationDocumentInt doc : notification.getDocuments()) {
            ...
            FileDownloadResponseInt fd = MDCUtils.addMDCToContextAndExecute(
                    safeStorageService.getFile(ref.getKey(), false)
                            .onErrorResume(PnFileNotFoundException.class, this::handleNotFoundError)).block();
            ...
            if (actualPages > maxPages) {
                throw new PnValidationMoreThan20GramsException(String.format(messageFormat,
                        notification.getSender().getPaTaxId(), fd.getKey(), actualPages, maxPages));
            }
        }
    }
    ...
    private Mono<FileDownloadResponseInt> handleNotFoundError(PnFileNotFoundException ex) {
        return Mono.error(new PnValidationFileNotFoundException(ex.getMessage(), ex.getCause()));
    }

    private boolean haveSomePaymentsAttachment(NotificationInt notification) {
        return notification.getRecipients().stream()
                .filter(r -> !CollectionUtils.isEmpty(r.getPayments()))
                .anyMatch(r -> r.getPayments().stream().anyMatch(p -> Objects.nonNull(p.getF24()))
                        || r.getPayments().stream().anyMatch(p -> p.getPagoPA() != null && Objects.nonNull(p.getPagoPA().getAttachment())));
    }
}
